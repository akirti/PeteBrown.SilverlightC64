...
namespace PeteBrown.SilverlightC64
{
    internal partial class DigitalRenderer : SIDRenderer
    {
...
#else
	        for (int i=0; i<256; i++) {
	          resonanceLP[i] = (float)(227.755 - 1.7635 * i - 0.0176385 * i * i + 0.00333484 * i * i * i - 9.05683E-6 * i * i * i * i);
	          resonanceHP[i] = (float)(366.374 - 14.0052 * i + 0.603212 * i * i - 0.000880196 * i * i * i);
	        }
#endif
#endif
        ...
        public override void VBlank()
        {
            ...
            //{
            //    //byte[] bytes = BitConverter.GetBytes(sound_buffer[i]);
            //    bytes[0] = (byte)(sound_buffer[i] & 0xFF00 >> 8);
            //    bytes[1] = (byte)(sound_buffer[i] & 0x00FF);
        ...
        void init_sound()
        {
            ...
            _audioStream = new MemoryStream(sndbufsize * 2);    // warning magic number. *2 is due to 16 bits per sample
