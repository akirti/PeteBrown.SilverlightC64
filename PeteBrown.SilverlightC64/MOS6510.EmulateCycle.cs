        ...
        public void EmulateCycle()
        {
...
#if DEBUG_INSTRUCTIONS
            if (lastState == 0 && debugLogger.Enabled)
            {
                lastState = 0xff;
                debugLogger.DisAssemble((ushort)(pc-1), a, x, y, sp, StatusByte);
            }
#endif
            ...
            if (state == 0 && interrupt.ANY != 0)
            {
                ...
                else if (interrupt.NMI == ByteBool.True && (TheC64.CycleCounter - first_nmi_cycle >= 2))
                {
                    ...
                    state = 0x0010;
#if DEBUG_INSTRUCTIONS
                    debugLogger.PushLastInterrupt(InterruptType.NMI);
#endif
                }
                else if ((interrupt.VICIRQ == ByteBool.True || interrupt.CIAIRQ == ByteBool.True) && (TheC64.CycleCounter - first_irq_cycle >= 2) && !i_flag)
                {
                    state = 0x0008;
#if DEBUG_INSTRUCTIONS
                    debugLogger.PushLastInterrupt(interrupt.VICIRQ == ByteBool.True ? InterruptType.VICIRQ : InterruptType.CIAIRQ);
#endif  
                }
#else
                if (interrupt.RESET == ByteBool.True)
            ...
            switch ((CPUState)state)
            {
                ...
                case CPUState.O_EXT:
                    ...
                    {
                        case 0x00:
                            _ram[0x90] |= TheIEC.Out(_ram[0x95], (_ram[0xa3] & 0x80) != 0);
                            c_flag = false;
                            pc = 0xedac;
                            state = 0; break; // Last macro;
                        case 0x01:
                            _ram[0x90] |= TheIEC.OutATN(_ram[0x95]);
                            c_flag = false;
                            pc = 0xedac;
                            state = 0; break; // Last macro;
                        case 0x02:
                            _ram[0x90] |= TheIEC.OutSec(_ram[0x95]);
                            c_flag = false;
                            pc = 0xedac;
                            state = 0; break; // Last macro;
                        case 0x03:
                            _ram[0x90] |= TheIEC.In(ref a);
                            z_flag = n_flag = a;
                            c_flag = false;
                            pc = 0xedac;
                            state = 0; break; // Last macro;
